...
if (!isset($_POST['EntrepreneurCheck']) && !isset($_POST['InvestisseurCheck'])) {
    $listOfErrors[] = "Vous devez sélectionner soit entrepreneur soit investisseur";
} else {
...
// Check if all role-specific fields are present
foreach ($roleFields as $field) {
    if (empty($_POST[$field])) {
        $listOfErrors[] = "Le champ " . $field . " est obligatoire pour votre rôle";
    }
}
